...
- **Model warmup on startup**: For Ollama models, tinycode sends a tool-call probe on startup (`src/provider/warmup.ts`) to pre-load the model into GPU memory and verify tool-calling support. In TUI mode, a toast shows warmup status. In direct mode, a footer message appears. The warmup uses `keep_alive: "10m"` to keep the model loaded between requests.
- **tc-doctor skill** (`src/skill/defaults/tc-doctor/`): Self-diagnostic skill with 14 checks covering Ollama install/health, model availability, tool-call capability probe, Mac-specific checks (Metal, Rosetta, RAM pressure), tinycode↔Ollama integration, vLLM provider health, and disk space. All checks use pure bash — no python3 dependency. Run with `/tc-doctor`.
