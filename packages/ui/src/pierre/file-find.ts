...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("tinycode-find")
  api.delete("tinycode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("tinycode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("tinycode-find", new Highlight(...rest))
    return true
  }
