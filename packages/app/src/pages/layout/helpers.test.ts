...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("tinycode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("tinycode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("tinycode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("tinycode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("tinycode://open-project")).toBeUndefined()
    expect(parseDeepLink("tinycode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "tinycode://open-project?directory=/a",
      "tinycode://other?directory=/b",
      "tinycode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("tinycode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("tinycode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("tinycode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("tinycode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "tinycode://new-session?directory=/a",
      "tinycode://open-project?directory=/b",
      "tinycode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __TINYCODE__: {
        deepLinks: ["tinycode://open-project?directory=/a"],
      },
    } as unknown as Window & { __TINYCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["tinycode://open-project?directory=/a"])
