...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.TINYCODE_REPO_CLONE_GITHUB_BASE_URL
      process.env.TINYCODE_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.TINYCODE_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.TINYCODE_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.read external_directory permission", () => {
  ...
  scout.live("does not ask for external_directory permission when reading configured references", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "tinycode-read-reference", "repo")
      ...
      const remoteDir = path.join(remoteRoot, "tinycode-read-reference")
      ...
      const dir = yield* tmpdirScoped({
        ...
        config: {
          reference: {
            docs: "tinycode-read-reference/repo",
          },
        },
      })
