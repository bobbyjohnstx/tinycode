...
it.live("local dev log is not truncated twice for the same run", () =>
  Effect.gen(function* () {
    const log = Global.Path.log
    const runID = process.env.TINYCODE_RUN_ID
    const initialized = process.env.TINYCODE_LOG_INITIALIZED_RUN_ID
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Global.Path.log = log
        if (runID === undefined) delete process.env.TINYCODE_RUN_ID
        else process.env.TINYCODE_RUN_ID = runID
        if (initialized === undefined) delete process.env.TINYCODE_LOG_INITIALIZED_RUN_ID
        else process.env.TINYCODE_LOG_INITIALIZED_RUN_ID = initialized
      }),
    )

    const dir = yield* tmpdirScoped()
    Global.Path.log = dir
    process.env.TINYCODE_RUN_ID = "run-1"
    delete process.env.TINYCODE_LOG_INITIALIZED_RUN_ID
