...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "tinycode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "tinycode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "tinycode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "tinycode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["tinycode.json", "tinycode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "tinycode.json"),
        path.join(tmp.path, "tinycode.jsonc"),
        path.join(project, "tinycode.json"),
        path.join(project, "tinycode.jsonc"),
      ])
    })
  })
