...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips tinycode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/test", {
          headers: {
            "x-tinycode-directory": "/secret/path",
            "x-tinycode-workspace": "ws_123",
            "x-custom": "preserved",
          },
        }),
      )
      ...
      expect(forwarded["x-tinycode-directory"]).toBeUndefined()
      expect(forwarded["x-tinycode-workspace"]).toBeUndefined()
