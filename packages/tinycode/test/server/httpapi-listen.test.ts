...
const original = {
  TINYCODE_SERVER_PASSWORD: Flag.TINYCODE_SERVER_PASSWORD,
  TINYCODE_SERVER_USERNAME: Flag.TINYCODE_SERVER_USERNAME,
  envPassword: process.env.TINYCODE_SERVER_PASSWORD,
  envUsername: process.env.TINYCODE_SERVER_USERNAME,
}
const auth = { username: "tinycode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.TINYCODE_SERVER_PASSWORD = original.TINYCODE_SERVER_PASSWORD
  Flag.TINYCODE_SERVER_USERNAME = original.TINYCODE_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.TINYCODE_SERVER_PASSWORD
  else process.env.TINYCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.TINYCODE_SERVER_USERNAME
  else process.env.TINYCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.TINYCODE_SERVER_PASSWORD = auth.password
  Flag.TINYCODE_SERVER_USERNAME = auth.username
  process.env.TINYCODE_SERVER_PASSWORD = auth.password
  process.env.TINYCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.TINYCODE_SERVER_PASSWORD = undefined
  Flag.TINYCODE_SERVER_USERNAME = auth.username
  delete process.env.TINYCODE_SERVER_PASSWORD
  process.env.TINYCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
