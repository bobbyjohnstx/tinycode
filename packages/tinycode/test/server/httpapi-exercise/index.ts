 ...
 *
 * The script intentionally isolates `TINYCODE_DB` before importing modules that touch
...
void (await import("@/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-tinycode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-tinycode-directory")
  }),
