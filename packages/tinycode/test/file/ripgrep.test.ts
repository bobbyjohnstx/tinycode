...
const tmpdir = (init?: (dir: string) => Effect.Effect<void>) =>
  Effect.acquireRelease(
    Effect.promise(async () => fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "tinycode-test-")))),
    (dir) =>
      Effect.promise(() =>
        fs.rm(dir, {
          recursive: true,
...
describe("file.ripgrep", () => {
    ...
    Effect.gen(function* () {
      const dir = yield* tmpdir((dir) =>
        Effect.gen(function* () {
          ...
          yield* mkdir(path.join(dir, ".tinycode"))
          yield* write(path.join(dir, ".tinycode", "thing.json"), "{}")
        }),
      )

      const files = yield* collectFiles({ cwd: dir })
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".tinycode", "thing.json"))).toBe(true)
    }),
  )
  ...
  it.live("hidden false excludes hidden", () =>
    Effect.gen(function* () {
      ...
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".tinycode", "thing.json"))).toBe(false)
    }),
  )
