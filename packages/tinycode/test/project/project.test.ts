...
import { Project as ProjectV2 } from "@/core/project"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const tinycodeFile = path.join(tmp, ".git", "tinycode")
      expect(yield* Effect.promise(() => Bun.file(tinycodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "tinycode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "tinycode")
      const wrongCache = path.join(parentDir, ".git", "tinycode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "tinycode")
      const cacheB = path.join(bareB, "tinycode")
      const wrongCache = path.join(parentDir, ".git", "tinycode")
