// Subprocess test harness for the tinycode CLI. Spawns the real binary against
...
//
// Configuration flows through tinycode's built-in test affordances:
//   - TINYCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - TINYCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - TINYCODE_DISABLE_PROJECT_CONFIG : skip walking up for tinycode.json
//   - TINYCODE_PURE                : skip external plugin discovery + install
//   - TINYCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
const tinycodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(tinycodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    TINYCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    TINYCODE_CONFIG_CONTENT: configJson,
    TINYCODE_DISABLE_PROJECT_CONFIG: "1",
    TINYCODE_PURE: "1",
    TINYCODE_DISABLE_AUTOUPDATE: "1",
    TINYCODE_DISABLE_AUTOCOMPACT: "1",
    TINYCODE_DISABLE_MODELS_FETCH: "1",
    TINYCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `tinycode run`. New flags should
...
// `tinycode serve` is a long-lived process — it never exits on its own.
...
// `tinycode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type TinycodeCli = {
  ...
  // Spawn `tinycode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("tinycode.spawn")(function* (args: string[], opts?: SpawnOpts) {
      ...
      // Use process.execPath so the spawned child finds bun even when it isn't
      // on PATH (e.g. ~/.bun/bin is not in the test runner's inherited PATH).
    ...
    const serve = Effect.fn("tinycode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `tinycode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("tinycode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `tinycode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "tinycode") {
...
// every call site. Use this for any test that needs the tinycode CLI fixture.
//
...
// resources (e.g. `tinycode.serve`) without an extra `Effect.scoped` wrapper —
