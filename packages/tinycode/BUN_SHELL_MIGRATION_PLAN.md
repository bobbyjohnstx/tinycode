...
Replace runtime Bun shell template-tag usage in `packages/tinycode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/tinycode/src` is removed except:
