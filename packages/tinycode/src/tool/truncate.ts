...
export interface Interface {
  ...
  /**
   * Resolved truncation limits: values from `tool_output` in tinycode config, or MAX_LINES / MAX_BYTES if unset.
   */
  readonly limits: () => Effect.Effect<{ maxLines: number; maxBytes: number }>
}

export class Service extends Context.Service<Service, Interface>()("@tinycode/Truncate") {}
  ...
  Effect.gen(function* () {
    ...
    const cleanup = Effect.fn("Truncate.cleanup")(function* () {
      ...
      for (const entry of entries) {
        ...
        if (!stat || stat.mtime.pipe(Option.map((t) => t.getTime()), Option.getOrElse(() => 0)) >= cutoff) continue
