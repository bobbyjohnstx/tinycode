...
// Keys injected by macOS/MDM into the managed plist that are not tinycode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/tinycode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "tinycode")
    default:
      return "/etc/tinycode"
  }
}
...
export function managedConfigDir() {
  return process.env.TINYCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
