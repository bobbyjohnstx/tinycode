import type { TuiPluginApi } from "@tinycode/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("tinycode.status"),
...
const TIPS: Tip[] = [
  ...
  "Use {highlight}/models{/highlight} to list and switch between all available models",
  ...
  "Run {highlight}/connect{/highlight} to configure Ollama, MaaS, or cloud providers",
  ...
  "Create {highlight}tinycode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/tinycode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.tinycode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.tinycode/agent/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.tinycode/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.tinycode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent tinycode from reading sensitive files",
  "Use {highlight}tinycode run{/highlight} for non-interactive scripting",
  "Use {highlight}tinycode --continue{/highlight} to resume the last session",
  "Use {highlight}tinycode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}tinycode serve{/highlight} for headless API access",
  "Use {highlight}tinycode run --attach{/highlight} to connect to a running server",
  "Run {highlight}tinycode agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.tinycode/themes/{/highlight} directory",
  ...
  "Run {highlight}tinycode debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} to configure your LLM provider (Ollama, MaaS, or cloud)",
