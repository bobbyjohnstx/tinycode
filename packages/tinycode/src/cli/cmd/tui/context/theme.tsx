...
import tinycode from "./theme/tinycode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@tinycode/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "tinycode")
        draft.active = typeof active === "string" ? active : "tinycode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.tinycode, store.mode)
    })
