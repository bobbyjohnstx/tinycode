...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from tinycode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const legacyFiles = await findLegacyConfigFiles(input)
...
async function findLegacyConfigFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "tinycode"),
    ...(await Filesystem.findUp(["tinycode.json", "tinycode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "tinycode"))
  }
  if (Flag.TINYCODE_CONFIG) files.push(Flag.TINYCODE_CONFIG)
