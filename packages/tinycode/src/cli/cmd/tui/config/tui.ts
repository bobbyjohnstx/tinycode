...
export class Service extends Context.Service<Service, Interface>()("@tinycode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old tinycode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.tinycode`
  // folders between cwd and home, and TINYCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.TINYCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit TINYCODE_TUI_CONFIG override, if set.
  if (Flag.TINYCODE_TUI_CONFIG) {
    const configFile = Flag.TINYCODE_TUI_CONFIG
  ...
  // 4. `.tinycode` directories (and TINYCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".tinycode") || dir === Flag.TINYCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".tinycode") && dir !== Flag.TINYCODE_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "tinycode.default",
