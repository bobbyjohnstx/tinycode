...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".tinycode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".tinycode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".tinycode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.TINYCODE_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.TINYCODE_PURE && config.plugin_origins?.length) {
