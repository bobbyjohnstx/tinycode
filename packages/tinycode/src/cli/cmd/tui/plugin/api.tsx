import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@tinycode/plugin/tui"
...
import type { useTinycodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useTinycodeKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getTinycodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getTinycodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
