...
const filterExperimentalServers = (servers: Record<string, LSPServer.Info>, flags: RuntimeFlags.Info) => {
  if (flags.experimentalLspTy) {
    if (servers["pyright"]) {
      log.info("LSP server pyright is disabled because TINYCODE_EXPERIMENTAL_LSP_TY is enabled")
...
export class Service extends Context.Service<Service, Interface>()("@tinycode/LSP") {}
