...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@tinycode/llm"
import type { LLMClientShape } from "@tinycode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("tinycode"))
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @tinycode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @tinycode/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
