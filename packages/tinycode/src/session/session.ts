...
import { NonNegativeInt, optionalOmitUndefined } from "@/core/schema"
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".tinycode", "plans")
...
export const getUsage = (input: { model: Provider.Model; usage: Usage; metadata?: ProviderMetadata }) => {
  ...
  const costInfo =
    ...
    (input.model.cost?.experimentalOver200K && contextTokens > 200_000
      ...
      : input.model.cost)
  const reportedCost =
    input.usage.cost !== undefined && Number.isFinite(input.usage.cost) && input.usage.cost >= 0
      ? input.usage.cost
      : undefined
  return {
    cost:
