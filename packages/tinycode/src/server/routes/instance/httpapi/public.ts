...
import { TinycodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for tinycode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(TinycodeHttpApi)
...
export const PublicApi = TinycodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "tinycode",
    version: "1.0.0",
    description: "tinycode api",
