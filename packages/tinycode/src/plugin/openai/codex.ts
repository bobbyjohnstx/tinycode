...
<html>
  <head>
    <title>tinycode - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>tinycode - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "tinycode"
      output.headers["User-Agent"] = `tinycode/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
