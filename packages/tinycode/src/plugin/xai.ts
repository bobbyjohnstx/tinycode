...
function authHeaders() {
  return {
    ...
    "User-Agent": `tinycode/${InstallationVersion}`,
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=tinycode` lets xAI attribute tinycode-originated
...
<html>
  <head>
    <title>tinyCODE - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>tinyCODE - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire tinycode process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `tinycode/${InstallationVersion}`)
