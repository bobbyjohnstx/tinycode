...
Practical reference for a future `packages/server` split after the tinycode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/tinycode`.
...
Until enough shared service code lives outside `packages/tinycode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/tinycode`

It should not import `packages/tinycode` services while `packages/tinycode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on tinycode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/tinycode` implementation details.
