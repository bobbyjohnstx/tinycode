...
Delete `packages/tinycode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/tinycode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/tinycode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/tinycode`
- TUI plugin loader tests in `packages/tinycode` if runtime plugin API wiring changed
