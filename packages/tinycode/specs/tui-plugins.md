...
- Author package entrypoint is `@tinycode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://tinycode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/tinycode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@tinycode-ai/plugin/tui`.
- `@tinycode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@tinycode-ai/plugin/tui"
...
- The value is a semver range checked against the running Tinycode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.tinycode` is absent, no check is performed (backward compatible).
...
- `tinycode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `tinycode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.tinycode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.tinycode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.tinycode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.tinycode` dir or `TINYCODE_CONFIG_DIR`, tinycode installs `@tinycode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@tinycode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus tinycode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Tinycode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, tinycode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@tinycode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"tinycode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `tinycode.default`.
...
- Local plugins persist installed themes under the local `.tinycode/themes` area near the plugin config source.
...
- `--pure` / `TINYCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.tinycode/plugins/tui-smoke.tsx`
- Local vim plugin: `.tinycode/plugins/tui-vim.tsx`
- Local smoke config: `.tinycode/tui.json`
- Local smoke theme: `.tinycode/plugins/smoke-theme.json`
