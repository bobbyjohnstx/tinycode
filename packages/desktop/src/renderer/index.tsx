...
import "./styles.css"
import { useTheme } from "@tinycode/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.TINYCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "tinycode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__TINYCODE__ ??= {}
  const pending = window.__TINYCODE__.deepLinks ?? []
  window.__TINYCODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("tinycode.global.dat").getItem("language")
