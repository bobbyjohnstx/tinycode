...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.tinycode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `tinycode.settings.dat` is special: it maps to the `tinycode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"tinycode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // tinycode.settings.dat → the electron settings store ("tinycode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "tinycode.global.dat").
  const storeName = filename === "tinycode.settings.dat" ? "tinycode.settings" : filename
